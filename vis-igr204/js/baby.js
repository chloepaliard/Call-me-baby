...
function displayOneName(){
    ...
    var y = d3.scaleLinear()
        .domain([0, d3.max(name_data, function(d) { return d.number; })])
...
$( function() {
    $( "#slider" ).slider({
        ...
        slide: function( event, ui ) {
            ...
            //Removing graph reset the selected name and displayingName so we have to save it
            let savedName = selected_name;
            let savedDisplaying = displayingName;
            removeGraph(); 
            selected_name = savedName;
            displayingName= savedDisplaying; 
            if (displayingName)
                displayOneName();          
            filter();
        }
...
function showSliderValue() {
    ...
    if (yearValue <= 2017 && yearValue >= 1900){
        $( function() {
            ...
            let savedName = selected_name;
            let savedDisplaying = displayingName;
            removeGraph(); //Remove graph reset the selected name to "" so we have to save it
            selected_name = savedName;
            displayingName= savedDisplaying;
            if (displayingName)
                displayOneName();   
